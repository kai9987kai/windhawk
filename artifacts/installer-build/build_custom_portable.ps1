param(
    ...
    [string]$FallbackRuntimeRoot = "C:\Program Files\Windhawk",
...
function Ensure-StagedRuntimeDirectory {
    param(
        [Parameter(Mandatory = $true)]
        [string]$StagingRoot,
        ...
        [string]$ValidationRelativePath
    )

    $stagedDirectory = Join-Path $StagingRoot $DirectoryName
    $validationPath = if ($ValidationRelativePath) {
        Join-Path $stagedDirectory $ValidationRelativePath
    } else {
        $stagedDirectory
    }
    ...
    $fallbackDirectory = if ($FallbackRoot) {
        Join-Path $FallbackRoot $DirectoryName
    } else {
        $null
    }
    $fallbackValidationPath = if ($fallbackDirectory -and $ValidationRelativePath) {
        Join-Path $fallbackDirectory $ValidationRelativePath
    } else {
        $fallbackDirectory
    }

    if ($fallbackDirectory -and (Test-Path $fallbackValidationPath)) {
        Write-Host ("   staging {0} from fallback runtime: {1}" -f $DirectoryName, $fallbackDirectory)
        Invoke-RobocopyMirror -Source $fallbackDirectory -Destination $stagedDirectory
        return
    }

    $fallbackDescription = if ($FallbackRoot) {
        "'$FallbackRoot'"
    } else {
        'no fallback runtime'
    }
    throw "Missing required runtime directory '$DirectoryName'. Baseline '$BasePortableRoot' does not contain it, and $fallbackDescription does not provide a usable replacement."
}
...
Invoke-Step -Message "Stage portable baseline" -Action {
    ...
    Ensure-StagedRuntimeDirectory `
        -StagingRoot $stagingRoot `
        -DirectoryName 'Compiler' `
        -FallbackRoot $FallbackRuntimeRoot `
        -ValidationRelativePath 'bin\clang++.exe'
